...
Mahdollistaa sen, että hyökkääjä voi pakottaa palvelimen lähettämään räätälöityjä pyyntöjä odottamattomiin kohteisiin ja ohittamaan palomuurit sekä verkon suojaukset, mikä voi johtaa sisäverkon skannaamiseen, arkaluonteisten paikallisten tiedostojen lukemiseen tai pilvipalveluiden metadatan vaarantumiseen. Yleisimpinä ongelmina ja syinä että käyttäjän URL-syötteisiin sokeasti luottaminen sekä puutteelliset verkkotason eristykset. Suojaudutaan segmentoimalla verkot ja asettamalla palomuureihin tiukat oletuksena estävät (deny by default) säännöt, validoimalla kaikki syötteet yksinomaan positiivisella sallittujen listalla (allow list) estolistojen (deny list) sijaan, estämällä HTTP-uudelleenohjaukset skeä varmistamalla, ettei sovellus välitä sisäisten pyyntöjen raakavastauksia suoraan takaisin käyttäjälle.
...
F) Avasin labran ja tarkistin ensimmäisen ZAP'ista Options --> Display ja täppä ruutuun "Process images in http requests/responses" jotta kuvien latauspyynöt on historiassa. Avasin labran sivuilta kuvan oheisen linkin <img width="2256" height="1906" alt="image" src="https://github.com/user-attachments/assets/ee5e6f2d-5e2a-4cf3-bfa9-fb889e576692" />
